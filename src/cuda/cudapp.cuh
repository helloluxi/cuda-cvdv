...
struct CudaAllocator : private Arena {
    ...
    template <class U>
    struct rebind {
        using other = CudaAllocator<U, Arena>;
    };
};
...
struct CudaFftPlan {
    ...
    CudaFftPlan() noexcept = default;
    CudaFftPlan(CudaFftPlan &&o) noexcept : m_plan(o.m_plan) { o.m_plan = 0; }
    CudaFftPlan &operator=(CudaFftPlan &&o) noexcept {
        std::swap(m_plan, o.m_plan);
        return *this;
    }
    CudaFftPlan(const CudaFftPlan &) = delete;
    CudaFftPlan &operator=(const CudaFftPlan &) = delete;

    operator cufftHandle() const noexcept { return m_plan; }
    explicit operator bool() const noexcept { return m_plan != 0; }

    static CudaFftPlan plan1d(int nx, cufftType type, int batch) {
        cufftHandle h;
        CHECK_CUFFT(cufftPlan1d(&h, nx, type, batch));
        return CudaFftPlan(h);
    }

    static CudaFftPlan plan2d(int nx, int ny, cufftType type) {
        cufftHandle h;
        CHECK_CUFFT(cufftPlan2d(&h, nx, ny, type));
        return CudaFftPlan(h);
    }

    static CudaFftPlan planMany(int rank, int *n, int *inembed,
                                int istride, int idist,
                                int *onembed, int ostride, int odist,
                                cufftType type, int batch) {
        cufftHandle h;
        CHECK_CUFFT(cufftPlanMany(&h, rank, n, inembed, istride, idist,
                                   onembed, ostride, odist, type, batch));
        return CudaFftPlan(h);
    }

    void exec(cuDoubleComplex *in, cuDoubleComplex *out, int direction) const {
        CHECK_CUFFT(cufftExecZ2Z(m_plan, in, out, direction));
    }

    ~CudaFftPlan() {
        if (m_plan) cufftDestroy(m_plan);
    }

private:
    explicit CudaFftPlan(cufftHandle h) noexcept : m_plan(h) {}
};
