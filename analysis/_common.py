...
def fit_ab(Gamma_vals, err_vals):
    """Fit log(err) = a*Gamma + b. Returns (a, b) or None."""
    if len(Gamma_vals) < 2:
        return None
    try:
        popt, _ = curve_fit(linear_model, Gamma_vals, np.log(err_vals))
        return tuple(popt)
    except Exception:
        return None
...
def plot_err_vs_gamma(df, x_col, y_col, group_col, ylabel, base_name, fig_dir, legend_loc='lower right'):
    ...
    for N, group in df.groupby(group_col):
        ...
        if ab is not None:
            ...
            ax.semilogy(Gf, np.exp(linear_model(Gf, *ab)), '--', linewidth=1.5, alpha=0.5)
